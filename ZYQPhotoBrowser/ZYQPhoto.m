...
+ (ZYQPhoto *)photoWithImage:(UIImage *)image {
	return [[ZYQPhoto alloc] initWithImage:image];
}

+ (ZYQPhoto *)photoWithFilePath:(NSString *)path {
	return [[ZYQPhoto alloc] initWithFilePath:path];
}

+ (ZYQPhoto *)photoWithURL:(NSURL *)url {
	return [[ZYQPhoto alloc] initWithURL:url];
}
...
+ (NSArray *)photosWithImages:(NSArray *)imagesArray {
    ...
    for (UIImage *image in imagesArray) {
        if ([image isKindOfClass:[UIImage class]]) {
            ZYQPhoto *photo = [ZYQPhoto photoWithImage:image];
...
+ (NSArray *)photosWithFilePaths:(NSArray *)pathsArray {
    ...
    for (NSString *path in pathsArray) {
        if ([path isKindOfClass:[NSString class]]) {
            ZYQPhoto *photo = [ZYQPhoto photoWithFilePath:path];
            [photos addObject:photo];
        }
    }
...
+ (NSArray *)photosWithURLs:(NSArray *)urlsArray {
    ...
    for (id url in urlsArray) {
        if ([url isKindOfClass:[NSURL class]]) {
            ...
            ZYQPhoto *photo = [ZYQPhoto photoWithURL:[NSURL URLWithString:url]];
            [photos addObject:photo];
        }
    }
...
#pragma mark ZYQPhoto Protocol Methods
